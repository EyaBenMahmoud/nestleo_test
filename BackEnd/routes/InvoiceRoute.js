...
// Get scheduled invoices with proper access control
router.get('/scheduled-invoices', protect, async (req, res) => {
  try {
    ...
    // Case 1: SyndicateAdmin can see their own buildings' invoices
    if (userRole === 'SyndicateAdmin') {
      // Get buildings where this user is the admin
      const adminBuildings = await Building.find({ user: userId }).select('_id');
      const adminBuildingIds = adminBuildings.map(building => building._id);
      
      // Filter invoices for these buildings
      query = { building: { $in: adminBuildingIds } };
    } 
    // Case 2: SuperAdmin or Admin can see all invoices
    else if (userRole === 'SuperAdmin' || userRole === 'Admin') {
      // No additional filtering needed - they see all
    } 
    // Case 3: SyndicateCoowner can only see their buildings' invoices
    else if (userRole === 'SyndicateCoowner') {
      // Get buildings where this user is a co-owner
      const coownerBuildings = await Building.find({ 
        coOwners: { $elemMatch: { $eq: userId } } 
      }).select('_id');
      const coownerBuildingIds = coownerBuildings.map(building => building._id);
      
      // Filter invoices for these buildings
      query = { building: { $in: coownerBuildingIds } };
    } 
    // Case 4: Other roles don't see any scheduled invoices
    else {
      return res.status(403).json({ error: 'Unauthorized to view scheduled invoices' });
    }

    // Execute the query with proper population
    const invoices = await RecurringInvoice.find(query)
      .populate('building')
      .populate('selectedBlocs')
      .populate('selectedApartments')
      .populate('currency')
    ...
    // Log for debugging
    console.log(`Found ${invoices.length} scheduled invoices for user ${userId} (${userRole})`);
