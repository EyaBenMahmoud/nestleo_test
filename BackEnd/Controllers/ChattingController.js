...
class ChatController {
  ...
  // Add this new method to the ChattingController class
      ...
      switch (user.role) {
        ...
        case 'SyndicateAdmin': {
          if (buildingId) {
            ...
            // Get workers that belong to this SyndicateAdmin
            ...
            // Get coOwners details
            const coOwners = await User.find({
              _id: { $in: coOwnerIds }
            }).select('firstName lastName avatar role email phoneNumber city country');

            // Combine all contacts
            contacts = [...coOwners, ...workers, ...adminUsers];
          } else {
            // If no building selected, show only Admins and Workers
            contacts = await User.find({
              ...baseQuery,
        ...
        case 'SyndicateCoowner': {
          try {
            ...
            if (buildingId) {
              ...
              // SyndicateAdmin of this building (building.user)
              const syndicateAdmin = building.user
                ? await User.findById(building.user)
                  .select('firstName lastName avatar role email phoneNumber city country')
                : null;

              // Other coOwners (excluding current user)
              const coOwnersList = building.coOwners.filter(
                co => co._id.toString() !== userId.toString()
              );

              contacts = syndicateAdmin ? [syndicateAdmin, ...coOwnersList] : coOwnersList;
            } else {
              ...
              const syndicateAdmins = await User.find({
                _id: { $in: syndicateAdminIds },
              }).select('firstName lastName avatar role email phoneNumber city country');

              contacts = syndicateAdmins;
            }
          } catch (error) {
            console.error('Error in SyndicateCoowner case:', error);
            return res.status(200).json({ contacts: [], error: 'Error fetching contacts' });
          }
          break;
        }
