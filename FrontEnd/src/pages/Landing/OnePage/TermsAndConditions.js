...
const TermsAndConditions = () => {
  ...
  document.title = t("termsAndConditions.title", "Terms and Conditions") + " | Nestleo";
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/crm/policy/termsAndConditions?lang=${currentLanguage}`);
                  ...
                  <div className="bg-soft-info position-relative">
                    <CardBody className="p-5">
                      <div className="text-center">
                        <h3>{t("termsAndConditions.heading", "Terms and Conditions")}</h3>
                        <p className="mb-0 text-muted">
                          {t("termsAndConditions.lastUpdate", "Last update")}: {formatDate(policy.lastUpdated)}
                        </p>
                        {fallbackLanguage && (
                          <div className="mt-2 text-warning">
                            <small>
                              {t("termsAndConditions.fallbackNotice", "This content is not available in your language and is shown in English.")}
                            </small>
                          </div>
                        )}
                      </div>
                  ...
                  <CardBody className="p-4">
                    {loading ? (
                      <div className="text-center py-5">
                        ...
                        <p className="mt-3">{t("termsAndConditions.loading", "Loading terms and conditions...")}</p>
                      </div>
                    ) : (
                      <>
