    ...
    <div style={{ marginTop: "71px" }}>
      ...
      {approachingLimit && (
        <Alert color="warning" className="mb-4">
          <div className="d-flex">
            ...
            <div>
              <h5>{t('apartmentWelcome.alerts.approachingLimitTitle')}</h5>
              <p className="mb-0">
                {t('apartmentWelcome.alerts.approachingLimitMessage')} {' '}
                {t('apartmentWelcome.alerts.upgradeSubscriptionMessage')}
              </p>
            </div>
          </div>
        </Alert>
      ...
      <Modal isOpen={coOwnerModal} toggle={() => setCoOwnerModal(false)} centered>
        ...
        <ModalBody>
          ...
          {/* Error message shown directly under the list when no co-owner is selected */}
          {!selectedCoOwner && filteredCoOwners.length > 0 && (
            <div className="mt-2 text-center text-danger">
              <i className="ri-error-warning-line me-1"></i>
              {t('apartmentWelcome.coOwnerModal.pleaseSelectCoowner')}
            </div>
          )}
        </ModalBody>
        <ModalFooter className="d-flex justify-content-between">
          {/* Cancel button on the left */}
          <Button color="light" onClick={() => setCoOwnerModal(false)}>
            {t('apartmentWelcome.modal.cancel')}
          </Button>

          {/* Assign button on the right */}
          <Button
            color="primary"
            onClick={handleAssignCoOwner}
            disabled={!selectedCoOwner}
          >
            {t('apartmentWelcome.coOwnerModal.assign')}
          </Button>
        </ModalFooter>
      </Modal>
      <DeleteModal
