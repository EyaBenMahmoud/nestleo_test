...
const calculateEndDate = (interval) => {
  ...
  router.post('/create-checkout-session', async (req, res) => {
      try {
          const { planId, customerEmail, price } = req.body; // Accept price from frontend
          ...
          // Ensure the Stripe Price ID exists for paid plans
          ...
          // If it's a free plan, handle it directly without Stripe
          if (plan.price === 0) {
              // Archive current subscription if exists and active
              ...
              user.subscription = {
                  ...
                  isTrial: false,
                  startDate: new Date(),
                  endDate: calculateEndDate(plan.interval || 'month')
              };
          ...
          // Use the calculated price from frontend if provided, otherwise use plan price
          const checkoutPrice = price ? (price / 10000) : plan.price; // Convert from cents if provided
          
          // For custom pricing, we need to create a one-time payment session instead of subscription
          // if the price differs from the original plan price
          if (checkoutPrice !== plan.price && checkoutPrice > 0) {
              // Create a one-time payment session for the difference
              const session = await stripe.checkout.sessions.create({
                  ...
                  line_items: [{
                      price_data: {
                          currency: 'usd',
                          product_data: {
                              name: `${plan.subscriptionType} - Upgrade`,
                              description: `Upgraded subscription with prorated pricing`,
                          },
                          unit_amount: Math.round(checkoutPrice * 100), // Convert to cents
                      },
                      quantity: 1,
                  }],
                  customer_email: customerEmail,
                  success_url: `${process.env.CLIENT_URL}/success?session_id={CHECKOUT_SESSION_ID}&plan_id=${planId}&user_id=${user._id}&is_upgrade=true`,
                  cancel_url: `${process.env.CLIENT_URL}/landing`,
                  metadata: {
                      planId: planId.toString(),
                      userId: user._id.toString(),
                      calculatedPrice: checkoutPrice.toString(),
                      isUpgrade: 'true'
                  },
              });
          ...
          // Create a standard Stripe Checkout Session for paid plans with original pricing
          const session = await stripe.checkout.sessions.create({
              payment_method_types: ['card'],
              mode: 'subscription',
              line_items: [{
  ...
  router.get('/confirm', async (req, res) => {
    try {
        ...
        // Handle upgrade payments (one-time payments for price differences)
        if (isUpgrade && session.mode === 'payment') {
            ...
            if (user.subscription && user.subscription.status === 'active') {
                user.subscriptionHistory.push({
                    ...
                    status: "inactive",
                    archivedAt: new Date(),
                    reason: "upgraded_with_proration"
                });
            }

            // Set new subscription without Stripe subscription ID (since it's a one-time payment)
            user.subscription = {
                planId,
                ...
                upgradePaymentId: session.payment_intent // Store payment intent for reference
            };
