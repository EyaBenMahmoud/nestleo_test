...
const addSubscription = async (req, res) => {
  try {
    ...
    if (typeof price === 'undefined' || price === null || subscriptionType === undefined || !interval) {
      return res.status(400).json({ error: "Missing required fields: subscriptionType, price, interval" });
    }
    ...
    let newSubscription;
    // If price is 0, do not create Stripe product/price
    if (price === 0) {
...
const updateSubscription = async (req, res) => {
  try {
    ...
    // Validate we have required Stripe references
    if (!existingSubscription.stripeProductId || !existingSubscription.stripePriceId) {
      return res.status(400).json({
        success: false,
        error: "Subscription missing Stripe references. Cannot update."
      });
    }
    ...
    if (interval && interval !== existingSubscription.interval) {
      const validIntervals = ["month", "year", "week", "day"];
      if (!validIntervals.includes(interval)) {
        return res.status(400).json({
          success: false,
      ...
      dbUpdates.interval = interval;
    }

    // Handle price update
    if (price && price !== existingSubscription.price) {
      if (isNaN(price)) {
        return res.status(400).json({
          success: false,
    ...
    // Step 1: Update Stripe Product if needed
    if (Object.keys(stripeProductUpdates).length > 0) {
      await stripe.products.update(existingSubscription.stripeProductId, stripeProductUpdates);
    }
