...
const PREDEFINED_FEATURES = [
  ...
  },
  {
    ...
    type: 'boolean', // Since this is just enabled/disabled
    defaultValues: {
      'Découverte': false,
      'Pro': true,
      'Expert': true
...
const SubscriptionModal = ({ isOpen, toggle, subscription, onSave, onClose }) => {
  ...
  // Add state for custom features
  const [customFeatures, setCustomFeatures] = useState([
    { name: '', isActive: true }
  ]);

  // Initialize features from predefined list
  useEffect(() => {
    if (subscription) {
      ...
      // Set custom features if any exist
  ...
  // Process existing features into predefined and custom
  const processExistingFeatures = (existingFeatures) => {
    ...
    existingFeatures.forEach(existingFeature => {
      ...
      const predefinedFeature = PREDEFINED_FEATURES.find(pf =>
        existingFeature.name.toLowerCase().includes(pf.name.toLowerCase())
      );

      if (predefinedFeature) {
        const index = mappedFeatures.findIndex(f => f.id === predefinedFeature.id);
        if (index !== -1) {
          // First check if there's metadata with the original value
          let value = 0;

          if (existingFeature.metadata && existingFeature.metadata.value !== undefined) {
            value = existingFeature.metadata.value;
          } else {
            // Try to extract value from the name, e.g., "Nombre d'immeubles: 5" -> 5
            const valueMatch = existingFeature.name.match(/:\s*(\d+|Illimité)/i);
            if (valueMatch) {
        ...
        // If it's not a predefined feature but doesn't have metadata, treat as custom
        extractedCustomFeatures.push({
          name: existingFeature.name,
          isActive: existingFeature.isActive
        });
  ...
  const initializeDefaultFeatures = () => {
    return PREDEFINED_FEATURES.map(feature => ({
      id: feature.id,
      name: feature.name,
      value: -1, // This is defaulting to -1 for unlimited
      isActive: true
    }));
  };

  // Add function to handle custom feature changes
  ...
  // Add function to add new custom feature field
  const addCustomFeature = () => {
    setCustomFeatures([...customFeatures, { name: '', isActive: true }]);
  };

  // Add function to remove custom feature field
  ...
  // Map existing features to predefined structure
  const mapExistingFeaturesToPredefined = (existingFeatures) => {
    const mappedFeatures = [...initializeDefaultFeatures()];

    existingFeatures.forEach(existingFeature => {
      // Check if feature name matches any predefined feature
      const predefinedFeature = PREDEFINED_FEATURES.find(pf =>
        existingFeature.name.toLowerCase().includes(pf.name.toLowerCase())
      );
  ...
  const handleFeatureChange = (index, e) => {
    ...
    if (type === 'checkbox') {
      ...
      // Handle "Illimité" special case
      if (value === t('subscriptions.unlimited') || value === 'Illimité') {
        updatedFeatures[index].value = -1;  // Store as -1 for unlimited
      } else {
        // For document size feature, strip the "Mo" suffix if present
  ...
  const formatFeaturesForSubmission = (predefinedFeatures) => {
    // Format predefined features - always store in French format
    const formattedPredefinedFeatures = predefinedFeatures.map(feature => {
      // Get the storage feature name (French)
      const storageFeatureName = getStorageFeatureName(feature.id);
      
      // Format feature name with value for display
      ...
      return {
        ...
        // Store the original ID and value as metadata for future editing
        metadata: {
          id: feature.id,
          value: feature.value,
          isPredefined: true
    ...
    // Format custom features
    const formattedCustomFeatures = customFeatures
      .filter(feature => feature.name.trim() !== '') // Filter out empty features
      .map(feature => ({
        name: feature.name,
        isActive: feature.isActive,
        metadata: {
  ...
  const getValueDisplay = (value) => {
    // Handle special cases for unlimited
    if (value === -1 || value === '-1') {
      return t('subscriptions.unlimited');
    }
    // Handle cases where value is 0 (inactive feature)
    if (value === 0 || value === '0') {
      return "0";
    }
  ...
  const getFeatureOptions = (featureId) => {
    // Return appropriate options based on feature type
    switch (featureId) {
      case 'numBuildings':
        return [
