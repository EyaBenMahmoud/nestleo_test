...
const ChatSidebar = ({
    ...
    const { contacts, chats, activeChatId, onlineUsers = [], availableUsersPagination } = useSelector((state) => state.chat);
...
const contactsListRef = useRef(null);
    // Add a ref for the SimpleBar component
const loadMoreUsers = () => {
    if (isLoadingMore || !availableUsersPagination?.hasNextPage) return;
    
    setIsLoadingMore(true);
    
    const nextPage = availableUsersPagination.currentPage + 1;
    console.log(`Loading more contacts - page ${nextPage}`);
    ...
    // Add this useEffect to handle scroll-based loading
    useEffect(() => {
        if (contactsListRef.current) {
            const scrollElement = contactsListRef.current.getScrollElement();

            const handleScroll = () => {
                const { scrollTop, scrollHeight, clientHeight } = scrollElement;

                // When user scrolls to bottom (with 20px threshold)
                if (scrollHeight - scrollTop - clientHeight < 20) {
                    if (!isLoadingMore && availableUsersPagination?.hasNextPage) {
                        loadMoreUsers();
                    }
                }
            };
    ...
    }, [isLoadingMore, availableUsersPagination, loadMoreUsers]);

    // Update the loadMoreUsers function to avoid duplicate calls
    ...
    // Add a useEffect to fetch contacts when the building changes
    // Update your useEffect for scroll detection
    // Remove this useEffect - we don't need it anymore
    useEffect(() => {
        if (customActiveTab === '2' && contactsListRef.current) {
            const scrollElement = contactsListRef.current.getScrollElement();

            const handleScroll = () => {
                const { scrollTop, scrollHeight, clientHeight } = scrollElement;

                // When user scrolls to within 50px of bottom
                if (scrollHeight - scrollTop - clientHeight < 50) {
                    if (!isLoadingMore && availableUsersPagination?.hasNextPage) {
                        console.log("Reached bottom, loading more contacts...");
                        loadMoreUsers();
                    }
                }
            };
    ...
    }, [customActiveTab, isLoadingMore, availableUsersPagination, loadMoreUsers]);
    // Format timestamp
    ...
    const handleScroll = () => {
        ...
        console.log(`Scroll position: ${Math.round(scrollTop + clientHeight)}/${scrollHeight}, ${scrollHeight - scrollTop - clientHeight}px from bottom`);
            ...
            <TabContent activeTab={customActiveTab} className="chat-tab-content">
                ...
                <TabPane tabId="2" id="contacts">
                    ...
                    >
                        ...
                        {/* Add this button outside the SimpleBar */}
                        {!loading && availableUsersPagination && availableUsersPagination.hasNextPage && (
                            <div className="text-center mt-3 mb-2">
                                ...
                                >
                                    {isLoadingMore ? (
                                        <><span className="spinner-border spinner-border-sm me-2"></span> {t('chat.loading')}</>
                                    ) : (
                                        <>{t('chat.loadMore')} <i className="ri-arrow-down-line ms-1"></i></>
                                    )}
                                </Button>
                            </div>
                        )}

                        {/* Keep the pagination info */}
                        {/* {!loading && availableUsersPagination && (
                            <div className="text-center text-muted small mb-3">
                                {t('chat.showing')} {contacts.length} {t('chat.of')} {availableUsersPagination.totalUsers} {t('chat.contacts')}
                            </div>
                        )} */}
                    </SimpleBar>
