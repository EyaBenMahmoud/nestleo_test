...
const RoleSelection = ({ t }) => {
  ...
  const handleContinue = (role) => {
    if (role === "SyndicateCoowner") {
      navigate("/registercoowner", { state: { role } });
    } else if (role === "SyndicateAdmin") {
      navigate("/registerSyndicate", { state: { role } });
    } else if (role === "Worker") {
      navigate("/registerWorker", { state: { role } });
    }
  };
            ...
            {activeRole && (
              <div className="continue-action">
                <button 
                  ...
                  onClick={() => handleContinue(activeRole.role)}
                >
...
export default withTranslation()(RoleSelection);