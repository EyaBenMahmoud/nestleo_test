...
const CookiePolicy = () => {
  ...
  document.title = t("cookiePolicy.title", "Cookie Policy") + " | Nestleo";
  ...
  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        ...
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/crm/policy/cookiePolicy?lang=${currentLanguage}`);
                  ...
                  <div className="bg-soft-info position-relative">
                    <CardBody className="p-5">
                      <div className="text-center">
                        <h3>{t("cookiePolicy.heading", "Cookie Policy")}</h3>
                        <p className="mb-0 text-muted">
                          {t("cookiePolicy.lastUpdate", "Last update")}: {formatDate(policy.lastUpdated)}
                        </p>
                        {fallbackLanguage && (
                          <div className="mt-2 text-warning">
                            <small>
                              {t("cookiePolicy.fallbackNotice", "This content is not available in your language and is shown in English.")}
                            </small>
                          </div>
                        )}
                      </div>
                  ...
                  <CardBody className="p-4">
                    {loading ? (
                      <div className="text-center py-5">
                        ...
                        <p className="mt-3">{t("cookiePolicy.loading", "Loading cookie policy...")}</p>
                      </div>
                    ) : (
                      <>
