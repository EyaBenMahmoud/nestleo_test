...
const checkForTimeConflicts = async (userId, startDate, eventTime, endTime, eventId = null) => {
  try {
    // Convertir les heures en minutes pour faciliter la comparaison
    const [startHour, startMinute] = eventTime.split(':').map(Number);
    const startTimeInMinutes = startHour * 60 + startMinute;

    // Même chose pour l'heure de fin si elle existe
    let endTimeInMinutes;
    if (endTime && endTime.trim() !== '') {
      const [endHour, endMinute] = endTime.split(':').map(Number);
      endTimeInMinutes = endHour * 60 + endMinute;
    } else {
      // Par défaut, on suppose une durée d'une heure
      endTimeInMinutes = startTimeInMinutes + 60;
    }

    // Obtenir la date au format YYYY-MM-DD pour la comparaison
    const dateString = new Date(startDate).toISOString().split('T')[0];

    // Trouver les événements du même jour créés par le même utilisateur
    const query = {
      user: userId,
      // On exclut l'événement en cours de modification s'il existe
      ...(eventId && { _id: { $ne: eventId } }),
      // On vérifie la même date
      start: {
    ...
    for (const event of existingEvents) {
      ...
      const [existingStartHour, existingStartMinute] = event.eventTime.split(':').map(Number);
      const existingStartTimeInMinutes = existingStartHour * 60 + existingStartMinute;

      let existingEndTimeInMinutes;
      if (event.endTime && event.endTime.trim() !== '') {
        const [existingEndHour, existingEndMinute] = event.endTime.split(':').map(Number);
        existingEndTimeInMinutes = existingEndHour * 60 + existingEndMinute;
      } else {
        existingEndTimeInMinutes = existingStartTimeInMinutes + 60;
      }
      ...
      if (
        ...
        (startTimeInMinutes <= existingStartTimeInMinutes && endTimeInMinutes >= existingEndTimeInMinutes)
      ) {
        return {
          hasConflict: true,
          conflictingEvent: {
...
const getEvents = async (req, res) => {
  try {
    ...
    if (req.user.role === 'SyndicateAdmin') {
      ...
      // Handle delegate events - find events where current user is a delegate
      ...
      if (usersWithDelegate && usersWithDelegate.length > 0) {
        const delegatorIds = usersWithDelegate.map(user => user._id);

        // Get apartments owned by delegators
        const delegatorApartments = await Apartment.find({
          coOwner: { $in: delegatorIds },
          building: buildingId
        });

        const delegatorApartmentIds = delegatorApartments.map(apt => apt._id);
        const delegatorBlocIds = delegatorApartments.map(apt => apt.bloc);

        // Find events where the delegators would have been invited
        const delegateEvents = await Event.find({
          building: buildingId,
          $or: [
            { isForAllCoOwners: true }, // For all co-owners (delegators included)
            { selectedCoOwners: { $in: delegatorIds } }, // Delegators specifically selected
            { selectedApartments: { $in: delegatorApartmentIds } }, // Delegator apartments
            { selectedBlocs: { $in: delegatorBlocIds } } // Delegator blocs
          ]
        })
          .populate('selectedCoOwners', 'firstName lastName email')
          .populate('selectedBlocs', 'name')
        ...
        // Mark delegate events and add delegator information
        const delegateEventsWithInfo = delegateEvents.map(event => {
          const eventObj = event.toObject();

          // Find which delegator(s) this event applies to
          const applicableDelegators = usersWithDelegate.filter(delegator => {
            // Check if delegator would be invited to this event
            if (event.isForAllCoOwners) return true;
            if (event.selectedCoOwners.some(coOwner => coOwner._id.toString() === delegator._id.toString())) return true;

            // Check delegator apartments
            const delegatorApts = delegatorApartments.filter(apt => apt.coOwner.toString() === delegator._id.toString());
            const delegatorAptIds = delegatorApts.map(apt => apt._id.toString());
            const delegatorBlocs = delegatorApts.map(apt => apt.bloc.toString());

            if (event.selectedApartments.some(apt => delegatorAptIds.includes(apt._id.toString()))) return true;
            if (event.selectedBlocs.some(bloc => delegatorBlocs.includes(bloc._id.toString()))) return true;

            return false;
          });

          // Add delegate information in the format expected by frontend
          eventObj.delegateInfo = {
            isDelegate: true,
            delegatorName: applicableDelegators.map(d => `${d.firstName} ${d.lastName}`).join(', '),
            delegatorId: applicableDelegators.length === 1 ? applicableDelegators[0]._id : null,
            delegators: applicableDelegators.map(delegator => ({
              _id: delegator._id,
        ...
        // Merge delegate events with user's own events, avoiding duplicates
        const existingEventIds = events.map(e => e._id.toString());
        const newDelegateEvents = delegateEventsWithInfo.filter(e =>
          !existingEventIds.includes(e._id.toString())
        );

        events = [...events, ...newDelegateEvents];
      }
    }
...
const updateEvent = async (req, res) => {
  ...
  try {
    ...
    if (startDateStartOfDay < todayStartOfDay) {
      ...
      const { hasConflict, conflictingEvent } = await checkForTimeConflicts(
        event.user.toString(),  // Utiliser l'ID utilisateur de l'événement
        parsedStartDate,
        eventTime,
        endTime,
        id  // Exclure l'événement en cours de modification
      );

      if (hasConflict) {
        // Récupérer les détails du bâtiment en conflit
        const conflictBuilding = await Building.findById(conflictingEvent.building);
        const buildingName = conflictBuilding ? conflictBuilding.name : 'Unknown building';

        return res.status(409).json({
          success: false,
          message: `Conflit d'horaire détecté avec un autre événement: "${conflictingEvent.title}" pour l'immeuble "${buildingName}" à ${conflictingEvent.time}.`,
          conflictingEvent
        });
      }
...
const createEvent = async (req, res) => {
  try {
    ...
    // NOUVELLE VÉRIFICATION: Recherche de conflits d'horaire
      const { hasConflict, conflictingEvent } = await checkForTimeConflicts(
        userId,
        parsedStartDate,
        eventTime,
