...
// Update the validateItemDescription function with stronger validation
const validateItemDescription = (value) => {
  ...
  if (value.trim().length < 3) {
    return t('invoices.recurring.errors.descriptionTooShort');
  }
  ...
  if (/[*/<>{}[\]\\]/.test(value)) {
    return t('invoices.recurring.errors.descriptionInvalidChars');
  }

  return '';
};

// Update validateName function for stronger validation
const validateName = (value) => {
  ...
  // Check for invalid characters
  if (/[*/<>{}[\]\\]/.test(value)) {
    return t('invoices.recurring.errors.nameInvalidChars');
  }
  
  // Check if name contains only special characters
  if (/^[^\w\s]+$/.test(value.trim())) {
    return t('invoices.recurring.errors.nameOnlySymbols');
  }
