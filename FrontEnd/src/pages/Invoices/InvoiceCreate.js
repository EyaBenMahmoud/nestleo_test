...
const InvoiceModal = ({ isOpen, toggle, invoiceId, onSuccess, currentBuilding }) => {
  ...
  // Replace your validateItems function with this improved version
  const validateItems = useCallback((items) => {
    ...
    items.forEach((item, index) => {
      ...
      if (!item.description || !item.description.trim()) {
        ...
        itemError.description = t("invoices.create.errors.descriptionTooShort");
        isValid = false;
      } else if (/^[^\w\s]+$/.test(item.description.trim())) {
        itemError.description = t("invoices.create.errors.descriptionOnlySymbols");
        isValid = false;
      } else if (/[*/<>{}[\]\\]/.test(item.description)) {
        itemError.description = t("invoices.create.errors.descriptionInvalidChars");
        isValid = false;
      }
      ...
      if (isNaN(amount) || amount === 0) {
        itemError.amount = t("invoices.create.errors.requiredAmounttt");
        isValid = false;
      } else if (amount < 0) {
        itemError.amount = t("invoices.create.errors.negativeAmount");
        isValid = false;
      }
...
const handleCurrencyChange = (selectedOption) => {
  ...
  const handleFormReset = useCallback(() => {
    ...
    setTaxRate(0.2);
  }, [user, currentBuilding, generateInvoiceNumber]);
...
// Add this useEffect to set initial currency when currencies load
useEffect(() => {
  if (currencies.length > 0 && !selectedCurrency && !formik.values.currency) {
