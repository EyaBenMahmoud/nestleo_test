            ...
            <Col lg={12}>
              <Card id="invoiceList">
                ...
                <CardBody className="pt-0">
                  ...
                  ) : (invoiceType === 'regular' && filteredInvoices.length === 0) ||
                    (invoiceType === 'scheduled' && filteredScheduledInvoices.length === 0) ? (
                    <div className="text-center py-5">
                      <div className="avatar-lg mx-auto mb-4">
                        <div className="avatar-title bg-light text-primary rounded-circle fs-24">
                          <i className={invoiceType === 'regular' ? "ri-file-list-3-line" : "ri-repeat-line"}></i>
                        </div>
                      </div>
                      <h5>{invoiceType === 'regular' ? t('invoices.noInvoices') : t('invoices.noScheduledInvoices')}</h5>
                      <p className="text-muted">
                        {searchText ? (
                          t('invoices.noSearchResults').replace('{searchTerm}', searchText)
                        ) : currentBuilding ? (
                          t('invoices.emptyBuildingMessage')
                        ) : (
                          t('invoices.selectBuildingToManage')
                        )}
                      </p>
                      ...
                      {invoiceType === 'regular' && (
                        <Button
                          ...
                          disabled={!currentBuilding || !canCreateInvoice}
                        >
                          <i className="ri-add-line align-bottom me-1"></i> {t('invoices.createFirstInvoice')}
                        </Button>
                      )}

                      {/* Create scheduled invoice button */}
                      {invoiceType === 'scheduled' && (
                        <Button
                          color="primary"
                          className="mt-3"
                          ...
                          disabled={!currentBuilding || !canCreateScheduledInvoice}
                        >
                          <i className="ri-repeat-line align-bottom me-1"></i> {t('invoices.createFirstScheduled')}
                        </Button>
                      )}
                    </div>
                  ) : (
