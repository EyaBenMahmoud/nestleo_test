...
// Replace just this thunk - leave everything else as is
export const fetchAvailableContacts = createAsyncThunk(
  'chat/fetchAvailableContacts',
  async (params, { rejectWithValue }) => {
    try {
      // Handle if params is just buildingId (for backward compatibility)
      let buildingId, page, limit, search;
      ...
      if (params && typeof params === 'object') {
        ...
        // If params is just the buildingId string
        buildingId = params;
        page = 1;
        limit = 10;
      }
      ...
      console.log(`Fetching contacts: page ${page}, limit ${limit}, buildingId: ${buildingId || 'none'}`);

      const response = await api.get(`/api/chats/contacts?${queryParams.toString()}`);
      return response.data;
    } catch (error) {
...
const initialState = {
  ...
  availableUsers: [],
  groupAvailableUsers: [],
  availableUsersPagination: null,  // Add this line
...
const chatSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Replace just this case inside your extraReducers
      .addCase(fetchAvailableContacts.fulfilled, (state, action) => {
        state.loading = false;

        // Handle both old and new response formats
        if (action.payload && action.payload.users && action.payload.pagination) {
          // New paginated format
          const { users, pagination } = action.payload;

          // If this is page 1, replace contacts array
          if (pagination.currentPage === 1) {
            state.contacts = users;
          } else {
            // Otherwise append to existing contacts (avoid duplicates)
            const existingIds = new Set(state.contacts.map(c => c._id));
            const newContacts = users.filter(c => !existingIds.has(c._id));
            state.contacts = [...state.contacts, ...newContacts];
          }

          // Store pagination data
          state.availableUsersPagination = pagination;
          console.log(`Loaded ${users.length} contacts, page ${pagination.currentPage}/${pagination.totalPages}`);
        } else {
          // Legacy format (array of contacts)
          state.contacts = action.payload || [];
          // Clear pagination
          state.availableUsersPagination = null;
        }
      })
